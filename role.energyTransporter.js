        ...
        if (creep.memory.state == "pickupEnergy") {
            if (target != undefined){
                ...
                if (temp != undefined){
                    ...
                    temp = creep.pos.findClosestByPath(FIND_STRUCTURES, {filter: (s) => (s.structureType == STRUCTURE_CONTAINER) && s.pos.getRangeTo(creep.pos) <= 5 && _.sum(s.store) > creep.carryCapacity - creep.carry.energy}); //Memory.structures.miningContainers);
                    if (temp != undefined){
